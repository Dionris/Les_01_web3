...
class Client:
    network: Network  # подсказки для main_03
    account: LocalAccount | None  # подсказки для main_03
    w3: Web3  # подсказки для main_03
    ...
    def __init__(
        ...
        if self.proxy:
            if 'http' not in self.proxy:
                self.proxy = f'http://{self.proxy}'  # проверка если в прокси не стоит http то мы дописываем
